...
body of response reveals the information about the created claim right: its internal
`id` (that matches the `Location` segment), its official `assetID` and
`dateModified` datestamp stating the moment in time when it was last
modified. Pay attention to the `assetType`. Note that the object is
